...
def preprocess_image(image_path):
    ...
    sr.readModel('./scalp_weights/EDSR_x3.pb')
    sr.setModel('edsr', 3)
    ...
    # numpy 배열을 cv2.UMat으로 변환
    image_umat = cv2.UMat(image_np)
    image_umat = sr.upsample(image_umat)
    # cv2.UMat을 numpy 배열로 변환
    image_np_upsampled = np.asarray(image_umat.get())
...
image_path = './scalp_weights/test.jpg'
